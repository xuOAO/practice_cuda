...
@triton.jit
def produce_quant_scale(x, eps):
    x_max = tl.max(x) 
    x_min = tl.min(x)
    x_max_abs = tl.abs(x_max)
    x_min_abs = tl.abs(x_min)
    quant_factor = tl.maximum(x_max_abs, x_min_abs)
    quant_factor = tl.clamp(quant_factor, eps)
...
def batch_quant_fp8_mm_kernel_base(
    ...
    for kk in tl.range(0, K, BLOCK_SIZE_K):
        ...
        # quanted
        quant_scale_a = produce_quant_scale(a)
        quant_scale_b = produce_quant_scale(b)
        a_fp8 = a / quant_scale_a
        b_fp8 = b / quant_scale_b

        accumulator += tl.dot(a_fp8, b_fp8)
