...
class SgemvBenchBase: public BenchBase {
public:
    SgemvBenchBase(int s, int k, bool check_correctness=true)
        : S(s), K(k), check_correctness(check_correctness){
        // [S, K] @ [K, 1]
        size_t size_a = S * K * sizeof(float);
        ...
        for (int i = 0; i < S; ++i) {
            ans_y[i] = 0.0f;
            for (int j = 0; j < K; ++j) {
                ans_y[i] += h_a[i * K + j] * h_x[j];
            }
        }
    }

    ~SgemvBenchBase() override {
        free(h_a);
        free(h_x);
        free(ans_y);
        cudaFree(d_a);
...
void func##_pipe_##name(kernel_name##BenchBase& bench, dim3 grid, dim3 block) { \
    auto kl = [&bench, grid, block]() { \
        int S = bench.S; \
        int K = bench.K; \
        name<<<grid, block>>>(bench.d_a, bench.d_x, bench.d_y, S, K); \
    }; \
...
void func##_pipe_##name##_##suffix(kernel_name##BenchBase& bench, dim3 grid, dim3 block) { \
    auto kl = [&bench, grid, block]() { \
        int S = bench.S; \
        int K = bench.K; \
        name<__VA_ARGS__><<<grid, block>>>(bench.d_a, bench.d_x, bench.d_y, S, K); \
    }; \
